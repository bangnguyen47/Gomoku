...
public class Game extends JFrame implements ActionListener, MouseListener,
		MouseMotionListener {
	private int[][] board; // [0][0] is top left; -1 = black, 0 = empty, 1 =
							// white
	...
	public void mouseClicked(MouseEvent e) {
		if (winner == 0 && e.getButton() == MouseEvent.BUTTON1) {
			...
			int row = (e.getY() - cell / 2) / cell, col = (e.getX() - cell / 2)
					/ cell;
			if (board[row][col] == 0) {
				...
				switch (winner) {
				case -1:
					side.setWinner("Player 1 wins!", Color.BLACK);
					break;
				case 1:
					side.setWinner("Player 2 wins!", Color.WHITE);
					break;
				case 2:
					side.setWinner("Draw!", Color.GREEN);
					break;
				}
				player = -player;
				side.setPlayer(player);
	...
	private void checkGameOver(int row, int col, int player) {
		...
		run = 0;
		for (int r = Math.max(row - col, 0), c = Math.max(col - row, 0); Math
				.max(r, c) < 19; r++, c++) {
			if (board[r][c] == player) {
				run++;
			} else {
			...
			for (int row = 0; row < 19; row++) {
				for (int col = 0; col < 19; col++) {
					if (board[row][col] == -1)
						...
						continue;
					g.fillOval(col * cell + cell / 2 + 1, row * cell + cell / 2
							+ 1, cell - 2, cell - 2);
				}
			}
		}
